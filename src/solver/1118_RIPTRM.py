...
class RIPTRM(Solver):
    ...
    def run(self, problem):
        ...
        # TRS_tolhardcase = option['TRS_tolhardcase']
        # checkTRSoptimality = option['checkTRSoptimality']
        # exit_warning_triggered = option['exit_warning_triggered']
        ...
        while True:
            ...
            def InnerIteration(x_initial, y_initial, mu, initial_TR_radius, costfun, ineqconstraints, manifold, inner_option):
                ...
                while True:
                    if verbosity > 1:
                        ...
                        print(f"Iteration: {iteration}-{inner_iteration}, Cost: {costfun(xCur)}, KKT resid: {manifold.norm(xCur, gradLagrangefun(xCur, yCur))}, TR_radius: {TR_radius}, {inner_status}")
                    inner_iteration += 1
                    ...
                    elif inner_iteration >= inner_maxiter:
                    ...
                    if checkTRSoptimality:
                        ...
                        """Eigendiffの計算ってこれでいいの？mineigvalHを単に使うの怪しい感じするけど"""
                        Cauchycond = True if Cauchydiff >= 0 else Cauchydiff
                        Eigencond = True if Eigendiff >= 0 else Eigendiff
            ...
            if save_inner_iteration:
                xPrev = inner_xPrev
            if inner_info["inner_status"] == "unconverged: max time exceeded" or inner_info["inner_status"] == "unconverged: max iter exceeded":
                xPrev = inner_xPrev
